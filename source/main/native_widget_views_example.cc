...
#if defined(OS_WIN)
  MyViewsDelegate* test_views_delegate =
      static_cast<MyViewsDelegate*>(MyViewsDelegate::views_delegate);
  views::View* old_default_parent_view =
      test_views_delegate->GetDefaultParentView();
  test_views_delegate->SetDefaultParentView(container);
#else
...
#if defined(OS_WIN)
  test_views_delegate->SetDefaultParentView(old_default_parent_view);
#endif
